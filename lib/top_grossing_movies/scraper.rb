...
class Scraper
  ...
  def self.scrape_movies # grab all the movies from imdb webpage list
    ...
    doc.css("div.lister-item.mode-detail").each do |movie|        #iterate through the content to pull
      scrape_movies << {                                     #add them into the movie array as a hash
...
movie_profile = movie.movie_profile
  def self.scrape_movie_profile(movie_profile)
    doc = Nokogiri::HTML(open("www.imdb.com#{movie_profile}"))
    movie_attributes = []
